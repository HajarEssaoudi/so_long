...
/*   Updated: 2025/03/17 02:16:42 by hes-saou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_xpm(t_game *game)
{
	...
	if (!game->mlx)
		(free_map(game), free(game), ft_putstr_fd("Error:\n mlx_init failed",
				2));
	game->window = mlx_new_window(game->mlx, (game->map_width) * XPM_WIDTH,
			(game->map_height) * XPM_HEIGHT, "so_long");
	if (!game->window)
		(free_map(game), free(game),
			ft_putstr_fd("Error: mlx_new_window failed", 2));
...
void	render_map(t_game *game)
{
	...
	while (game->map[y])
	{
		...
		while (game->map[y][x])
		{
			...
			if (game->map[y][x] == 'P')
				mlx_put_image_to_window(game->mlx, game->window,
					game->player_img, x * XPM_WIDTH, y * XPM_HEIGHT);
			else if (game->map[y][x] == 'E')
				mlx_put_image_to_window(game->mlx, game->window, game->exit_img,
					x * XPM_WIDTH, y * XPM_HEIGHT);
			else if (game->map[y][x] == 'C')
				mlx_put_image_to_window(game->mlx, game->window,
					game->collectible_img, x * XPM_WIDTH, y * XPM_HEIGHT);
			x++;
		}
		y++;
	}
